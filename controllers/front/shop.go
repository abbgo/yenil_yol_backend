...
func GetShopsForMap(c *gin.Context) {
	...
	rowsShopQuery := fmt.Sprintf(`
							...
							WHERE 6371 * acos(
									...
									) <= %d AND deleted_at IS NULL AND parent_shop_id IS NULL AND created_status=%d;
...
func GetShops(c *gin.Context) {
	...
	for rowsShop.Next() {
		...
		if err := rowsShop.Scan(&shop.ID, &shop.NameTM, &shop.NameRU, &shop.Latitude, &shop.Longitude, &shop.Image,
			&shop.AddressTM, &shop.AddressRU, &shop.ParentShopID, &shop.IsShoppingCenter); err != nil {
			helpers.HandleError(c, 400, err.Error())
			return
		}
...
func GetShopByID(c *gin.Context) {
	...
	var shop models.Shop
	db.QueryRow(context.Background(), "SELECT id,name_tm,name_ru,address_tm,address_ru,latitude,longitude,resized_image,has_shipping FROM shops WHERE id = $1 AND deleted_at IS NULL", shopID).Scan(&shop.ID, &shop.NameTM, &shop.NameRU, &shop.AddressTM, &shop.AddressRU, &shop.Latitude, &shop.Longitude, &shop.Image, &shop.HasShipping)
