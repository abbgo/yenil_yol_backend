...
func UpdateProductByID(c *gin.Context) {
	...
	_, err = db.Exec(
		...
		slug_tm=$6 , slug_ru=$7 , brend_id=$8 , is_visible=$9 , shop_id=$10 , created_status=0 , 
		rejected_reason=NULL , genders=$11 WHERE id=$12`,
		...
		product.BrendID, product.IsVisible, product.ShopID, pq.Array(product.Genders), product.ID)
	if err != nil {
		helpers.HandleError(c, 400, err.Error())
		return
	}
...
func GetProducts(c *gin.Context) {
	...
	rowQuery := fmt.Sprintf(`SELECT p.id,p.name_tm,p.name_ru,p.price,p.old_price,p.is_visible,p.created_status,p.rejected_reason FROM products p WHERE p.deleted_at IS %v`, isDeleted)
